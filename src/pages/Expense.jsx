...
export default function Expense() {
      ...
      try {
        ...
        const safe = Array.isArray(data)
          ? data.map((e, idx) => ({
              ...
              status: e.status === true || e.isSettled === true, // settled?
              ...
              members: Array.isArray(e.members)
                ? e.members.map((m) => ({
                    ...
                    memberId: m.memberId ?? m.userId ?? m.member_id ?? m.user_id ?? null,
                    splitId:
                      m.splitId ??
                      m.split_id ??
                      m.s_id ??
        ...
        if (e.name !== "AbortError") setErr(e.message || "Something went wrong while fetching.");
      } finally {
        setLoading(false);
      }
    })();
  ...
  // Visible list:
  // - always hide settled
  // - personal: also hide items where *my* split is already paid
  const visible = useMemo(() => {
    const base = expenses.filter((e) => !e.status); // only unsettled expenses
    if (show !== "personal") return base;
    return base.filter((e) => {
      const mine = e.members?.find((m) => String(m.memberId) === String(memberId));
      // if my split exists and it's already paid -> hide
      if (mine && mine.paid) return false;
      return true; // keep if no mine or mine unpaid
    });
  }, [expenses, show, memberId]);

  // Group visible by yyyy-mm-dd
  ...
  // Pay my share → PUT /api/v1/split/markPaid/{splitId}
    ...
    <div className="relative min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="flex-1 px-5 pt-20 pb-28 text-slate-800 max-w-md mx-auto w-full">
        ...
        {loading ? (
          ...
          <>
            {/* — Visible (unsettled; plus personal hides already-paid-by-me) grouped by date — */}
            <section className="space-y-6">
              {groups.length === 0 ? (
                <div className="text-sm text-slate-500 italic">
                  {show === "personal" ? "No unpaid items for you 🎉" : "No unsettled expenses 🎉"}
                </div>
              ) : (
                groups.map((g) => (
                      ...
                      {g.rows.map((exp) => (
                        ...
                        >
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              <h3 className="font-semibold text-slate-900 truncate">{exp.title}</h3>
                            ...
                            <div className="mt-1 text-xs text-slate-500">
                              ...
                              <span className="text-slate-700 font-medium">{exp.paidBy}</span>
                            </div>
                          </div>

                          <div className="text-right shrink-0 pl-3">
                            <div className="text-sm font-bold text-slate-900">{fmtMYR(exp.amount)}</div>
                          </div>
                        </button>
                      ))}
                    </div>
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-slate-100 text-slate-700 text-[11px] px-2.5 py-1">
                    ...
                    Paid by{" "}
                    <span className="font-medium text-slate-900">{selected.paidBy}</span>
                  </div>
                </div>
                ...
                <ul className="divide-y divide-slate-200">
                  {selected.members.filter((m) => m.paid).length === 0 ? (
                    ...
                    selected.members
                      .filter((m) => m.paid)
                      .map((m, i) => (
                        <li key={`paid-${i}`} className="px-4 py-3 flex items-center justify-between">
                          <span className="text-[15px] text-slate-800">{m.name}</span>
              ...
              <section className="rounded-xl border border-slate-200">
                ...
                <ul className="divide-y divide-slate-200">
                  {selected.members.filter((m) => !m.paid).length === 0 ? (
                    ...
                    selected.members
                      .filter((m) => !m.paid)
                      .map((m, i) => (
                        <li key={`unpaid-${i}`} className="px-4 py-3 flex items-center justify-between">
                          <span className="text-[15px] text-slate-800">{m.name}</span>
                          <span className="text-[15px] font-semibold text-slate-900 tabular-nums">
                            RM {Number(m.amount).toFixed(2)}
                          </span>
                ...
                <div className="px-4 py-3 bg-slate-50 border-t border-slate-200">
                  ...
                  {mySplitInSelected ? (
                    ...
                    <div className="text-xs text-slate-600">Your member record is not part of this split.</div>
                  )}
                </div>
              </section>
              ...
              {selected.notes && (
                <section className="rounded-xl border border-slate-200 bg-slate-50">
                  ...
                  <div className="px-4 py-3">
                    <p className="text-sm leading-relaxed text-slate-700 whitespace-pre-wrap">{selected.notes}</p>
                  </div>
                </section>
              )}
            </div>
